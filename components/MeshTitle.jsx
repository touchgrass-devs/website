...
// Mesh-warp title: renders `text` onto a WebGL grid that ripples away from the
// cursor on hover (drag + spring physics), with an optional chromatic-fringe
// split. Adapted from a Framer/Originkit "Mesh Text Hover" component - ported
// to plain JS/React, given left-aligned multi-line text wrapping (the source
// only supported a single centered line), and recolored to this site's own
// palette instead of the original neon pink/green split. Falls back to plain
// static text if WebGL2 isn't available.
...
function compile(gl, type, src) {
  ...
  if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
    console.error('MeshTitle shader compile error:', gl.getShaderInfoLog(sh));
    gl.deleteShader(sh);
    return null;
  }
  return sh;
...
function linkProgram(gl, vs, fs) {
  ...
  if (!gl.getProgramParameter(p, gl.LINK_STATUS)) {
    console.error('MeshTitle program link error:', gl.getProgramInfoLog(p));
    gl.deleteProgram(p);
    return null;
  }
  return p;
...
// Left-aligned, word-wrapped (the source component only drew one centered
// line via a single fillText call, which clips/overflows on real multi-word
// titles) so it matches the left-aligned title it replaces in the card.
function renderTextToCanvas(text, color, fontFamily, fontWeight, fontStyle, fontSize, width, height) {
  ...
  lines.forEach((line, i) => {
    ctx.fillText(line, leftPad, startY + i * lineHeight);
  });
  return c;
}

export default function MeshTitle({
  text,
  color = '#111827',
  ...
  useEffect(() => {
    ...
    const rebuildTex = async () => {
      ...
      if (cancelled) return;
      const c2 = renderTextToCanvas(text, color, fontFamily, fontWeight, fontStyle, realSize, w, h);
      gl.bindTexture(gl.TEXTURE_2D, tex);
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} style={{ display: supported ? 'block' : 'none', width: '100%', height: '100%' }} />
      {!supported && (
        <div
          style={{
            fontFamily,
